...
namespace Waffle {
	...
	#define WF_INTERNAL_ASSERT_NO_MSG(type, check) WF_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", WF_STRINGIFY_MACRO(check), ::Waffle::ExtractFileName(__FILE__), __LINE__)
...
#define WF_CORE_VERIFY(condition, ...) do { if(!(condition)) { WF_CORE_ERROR("Verify Failed (" __FILE__ ":" WF_STRINGIFY_MACRO(__LINE__) ") " __VA_ARGS__); WF_DEBUGBREAK(); } } while(0)
#define WF_VERIFY(condition, ...) do { if(!(condition)) { WF_ERROR("Verify Failed (" __FILE__ ":" WF_STRINGIFY_MACRO(__LINE__) ") " __VA_ARGS__); WF_DEBUGBREAK(); } } while(0)

#define WF_CORE_ENSURE(condition, ...) [&]{ if(!(condition)) { WF_CORE_ERROR("Ensure Failed (" __FILE__ ":" WF_STRINGIFY_MACRO(__LINE__) ") " __VA_ARGS__); WF_DEBUGBREAK(); } return (condition); }()
#define WF_ENSURE(condition, ...) [&]{ if(!(condition)) { WF_ERROR("Ensure Failed (" __FILE__ ":" WF_STRINGIFY_MACRO(__LINE__) ") " __VA_ARGS__); WF_DEBUGBREAK(); } return (condition); }()