...
namespace std {
	...
	template <>
	struct formatter<Waffle::UUID> : formatter<uint64_t>
	{
		template <typename FormatContext>
		FormatContext::iterator format(const Waffle::UUID id, FormatContext& ctx) const
		{
	...
	struct formatter<glm::quat>
	{
		...
		{
			return format_to(ctx.out(), "({:.3f}, {:.3f}, {:.3f}, {:.3f})", q.w, q.x, q.y, q.z);
		}
	};
